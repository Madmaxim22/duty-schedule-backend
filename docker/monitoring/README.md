...
sh scripts/fetch-grafana-dashboards.sh   # Node Exporter 11074, PostgreSQL 14114
...
git pull   # скрипт качает 11074, 14114
sh scripts/fetch-grafana-dashboards.sh
...
**`Datasource ${DS_PROMETHEUS} was not found`** / **`${DS_GDEV-PROMETHEUS}`** — community-JSON с grafana.com содержит плейсхолдеры `${DS_*}`; при file provisioning они не подставляются. Перекачайте дашборды (скрипт прописывает uid `prometheus`):
...
Дашборды: **Duty → Docker Containers**. Community **Cadvisor exporter** не используйте.
