import { appReleaseId } from '../../lib/app-version.js';
import type { ReleaseNotes } from './releases.types.js';

export type { ReleaseNotes } from './releases.types.js';

export const RELEASES: Record<string, ReleaseNotes> = {
  '1.0.0': {
    id: '1.0.0',
    version: '1.0.0',
    title: 'Первая версия',
    publishedAt: '2026-05-17',
    items: [
      'Календарь дежурств и личный график',
      'Оповещения об изменениях графика',
      'Галерея фото профиля и лайки',
    ],
  },
  '1.1.0': {
    id: '1.1.0',
    version: '1.1.0',
    title: 'Чат и поддержка',
    publishedAt: '2026-05-28',
    items: [
      'Чаты между сотрудниками',
      'Обращения в техподдержку',
      'Push-уведомления в браузере',
    ],
  },
  '1.2.0': {
    id: '1.2.0',
    version: '1.2.0',
    title: 'Обмен дежурствами',
    publishedAt: '2026-06-01',
    items: [
      'Заявки на обмен дежурствами между сотрудниками',
      'Согласование обмена и уведомления в чате',
      'Раздел «Обновления» с историей версий',
    ],
  },
  '1.3.0': {
    id: '1.3.0',
    version: '1.3.0',
    title: 'Видео в чате',
    publishedAt: '2026-06-07',
    items: [
      'Отправка видео в чате (mp4, webm, mov, до 80 МБ)',
      'Превью и просмотр видео в сообщении',
      'Калькулятор денежного довольствия в меню приложения',
      'Обновлены пороги месячных достижений: «Надёжный дежурный» — 3–6 дежурств, «Герой графика» — от 7',
      'В итогах месяца — слова поддержки, если вы были на больничном',
      'На планшете и компьютере приложение растягивается на всю ширину экрана',
      'Вид «Таблица» на главной: график по сотрудникам и датам; администратор может назначать дежурства кликом и перетаскиванием',
      'На планшете и компьютере профиль открывается в широком окне с галереей; фото листаются как в чате',
      'В калькуляторе довольствия — выбор руководящей должности по Прил. № 9 с автоматическим %',
      'В калькуляторе довольствия — автонадбавка 50% для 1–4 разряда, разовые достижения отдельными строками',
    ],
  },
};

export const CURRENT_RELEASE_ID = appReleaseId;

export function getCurrentRelease(): ReleaseNotes | null {
  return RELEASES[CURRENT_RELEASE_ID] ?? null;
}
